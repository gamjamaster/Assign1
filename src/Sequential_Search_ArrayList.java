...
class Sequential_Search_ArrayList<T> implements Runnable {
    ...
    public Sequential_Search_ArrayList(List<T> list, T target, AtomicLong time) {
        this.list = list;
        this.target = target;
        this.time = time;
    }
    ...
    System.out.println("Sequential Search (ArrayList) finished in " + executionTime/1_000_000.0 + " ms, index=" + result);
}
